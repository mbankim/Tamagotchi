...
uint8_t danceGameStart() {
  ...
  drawString("Copy the Pattern", 0, 0);
  updateScreen();
  delay(2000);
...
uint8_t danceGameEnd() {
  if (dance_lose) {
    ...
    updateScreen();
    delay(2000);
    return dance_level*5;
  }
  else {
    drawString("You Win!", 5, 15);
    updateScreen();
    delay(2000);
    return dance_level*20;
  }
}



void danceReceiveInput() {
  ...
  //check shaking (must shake for long enough to ensure the user is shaking)
